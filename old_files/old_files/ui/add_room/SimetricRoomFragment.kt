...
import kotlinx.android.synthetic.main.fragment_simetric_room.*
import kotlinx.coroutines.launch
import org.koin.androidx.viewmodel.ext.android.viewModel

class SimetricRoomFragment : BaseFragment() {
    ...
    private val viewModel: SimetricRoomViewModel by viewModel()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_simetric_room, container, false)
    }
    ...
    private fun setupView() {
        setActionBarWithButtonHome(getString(R.string.simetric_room))
        tv_note.text = roomName
    }

    private fun setupListener() {
        btn_add_simtericRoom.setOnClickListener {
            addRoom()
            this.hideKeyboard()
        }
    }
    ...
    private fun addRoom() {
        if ( !edt_width.text.isEmpty()&&
                !edt_height.text.isEmpty() &&
                !edt_compr.text.isEmpty()
        ) {
            setupViewData()
        } else {
    ...
    private fun setupViewData() {
        ...
        val room = Room(null,
                roomId.toInt(), orderNum, 2, edt_note.text.toString(), switchm.isChecked,
                ...
                spinner_demao_qty.selectedItemPosition, edt_width.toFloat(), edt_height.toFloat(), edt_compr.toFloat(), 0f, 0f, 0f)
        createRoom(room)
    }

    fun createRoom(room: Room) = lifecycleScope.launch {
       try {
           viewModel.insertSimetricRoom(room)
