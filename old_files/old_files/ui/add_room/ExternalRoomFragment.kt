...
import kotlinx.android.synthetic.main.fragment_external_room.*
import kotlinx.coroutines.launch
import org.koin.androidx.viewmodel.ext.android.viewModel


class ExternalRoomFragment : BaseFragment() {
    ...
    private val viewModel: ExternalRoomViewModel by viewModel()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_external_room, container, false)
    }
    ...
    private fun setupView() {
        setActionBarWithButtonHome(getString(R.string.external_room))
    ...
    private fun addRoom() {
        if (!edt_height.text.isEmpty() && !edt_width.text.isEmpty()) {
            setupViewData()
        } else {
    ...
    private fun setupViewData() {
        val room = Room(null,
                ...
                spinner_demao_qty.selectedItemPosition, edt_width.toFloat(), edt_height.toFloat(), 0f, 0f, 0f, 0f)
        createRoom(room)
    }

    fun createRoom(room: Room) = lifecycleScope.launch {
        try {
            viewModel.insertExternalRoom(room)
